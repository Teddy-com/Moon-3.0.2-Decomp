...
extends TabComponent {
    ...
    public void onDraw(ScaledResolution scaledResolution) {
        ...
        if (this.categoryComponent.getSelectedModule() == this.module) {
            this.transUtil.interpolate(this.getPosX() + 8.0f, 0.0, 15.0f / (float)Minecraft.getDebugFPS());
        } else {
            this.transUtil.interpolate(this.getPosX() + 4.0f, 0.0, 15.0f / (float)Minecraft.getDebugFPS());
        }
        if (this.module.isEnabled()) {
            Fonts.moonLittleBiggerBold.drawString(this.module.getLabel(), (float)this.transUtil.getPosX(), this.getPosY(), -1);
        } else {
            Fonts.moonLittleBigger.drawString(this.module.getLabel(), (float)this.transUtil.getPosX(), this.getPosY(), -1);
        }
