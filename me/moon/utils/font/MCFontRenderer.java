...
import me.moon.utils.font.FontRender;
...
extends FontRender {
    protected FontRender.CharData[] boldChars = new FontRender.CharData[256];
    protected FontRender.CharData[] italicChars = new FontRender.CharData[256];
    protected FontRender.CharData[] boldItalicChars = new FontRender.CharData[256];
    ...
    public float drawStringWithShadow(String text, double x, double y, int color, int shadowColor) {
        float shadowWidth = this.drawString(text, x + 0.5, y + 0.5, color, shadowColor, true);
        return Math.max(shadowWidth, this.drawString(text, x, y, color, shadowColor, false));
    }

    public void drawOutlinedString(String text, double x, double y, int color) {
        this.drawStringNoColor(text, x - 0.5, y);
        this.drawStringNoColor(text, x + 0.5, y);
        this.drawStringNoColor(text, x, y + 0.5);
        this.drawStringNoColor(text, x, y - 0.5);
    ...
    public float drawCenteredStringWithShadow(String text, float x, float y, int color) {
        float shadowWidth = this.drawString(text, (double)(x - (float)(this.getStringWidth(text) / 2)) + 0.6, (double)y + 0.6, color, true);
        return this.drawString(text, x - (float)(this.getStringWidth(text) / 2), y, color);
    }

    public float drawStringNoColor(String text, double x, double y) {
        x -= 1.0;
        if (text == null) {
            return 0.0f;
        }
        int color = Color.black.getRGB();
        FontRender.CharData[] currentData = this.charData;
    ...
    public float drawString(String text, double x, double y, int color, boolean shadow) {
        ...
        if (shadow) {
            float normalColorAlpha = new Color(color).getAlpha();
            color = new Color(0.1f, 0.1f, 0.1f, normalColorAlpha / 255.0f).getRGB();
        }
        FontRender.CharData[] currentData = this.charData;
        float alpha = (float)(color >> 24 & 0xFF) / 255.0f;
        boolean bold = false;
    ...
    public float drawString(String text, double x, double y, int color, int shadowColor, boolean shadow) {
        x -= 1.0;
        if (text == null) {
            return 0.0f;
        }
