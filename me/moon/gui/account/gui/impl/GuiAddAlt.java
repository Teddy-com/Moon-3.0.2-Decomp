...
extends GuiScreen {
    ...
    private GuiPasswordField passemail;
    ...
    public void initGui() {
        ...
        this.buttonList.add(new GuiButton(0, this.width / 2 - 100, this.height / 2, 200, 20, "Add", true));
        this.buttonList.add(new GuiButton(1, this.width / 2 - 100, this.height / 2 + 24, 200, 20, "Back", true));
        this.passemail = new GuiPasswordField(1, this.fontRendererObj, this.width / 2 - 100, this.height / 2 - 48, 200, 20);
        this.passemail.setMaxStringLength(Integer.MAX_VALUE);
    }

    @Override
    public void keyTyped(char character, int keyCode) {
        this.passemail.textboxKeyTyped(character, keyCode);
        switch (keyCode) {
            case 28: {
    ...
    protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
        ...
        this.passemail.mouseClicked(mouseX, mouseY, mouseButton);
    }

    @Override
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        RenderUtil.drawRect(0.0, 0.0, this.mc.displayWidth, this.mc.displayHeight, -14606047);
        Fonts.sectioNormal.drawCenteredStringWithShadow("Add Alt", this.width / 2, 5.0f, -1);
        this.passemail.drawTextBox();
        if (this.passemail.getText().isEmpty() && !this.passemail.isFocused()) {
            Fonts.sectioNormal.drawStringWithShadow("Email:Pass", this.passemail.xPosition + 4, (double)this.passemail.yPosition + 7.5, -7829368);
        }
    ...
    protected void actionPerformed(GuiButton button) {
        switch (button.id) {
            case 0: {
                if (!this.passemail.getText().contains(":")) break;
                String[] args = this.passemail.getText().split(":");
                Account acc = new Account(args[0], args[1]);
                if (Moon.INSTANCE.getAccountManager().getAccounts().contains(acc)) break;
                Moon.INSTANCE.getAccountManager().getAccounts().add(acc);
    ...
    public void updateScreen() {
        this.passemail.updateCursorCounter();
    }
}
