...
implements IBlockAccess {
    ...
    public BlockPos getHeight(BlockPos pos) {
        int i = pos.getX() >= -30000000 && pos.getZ() >= -30000000 && pos.getX() < 30000000 && pos.getZ() < 30000000 ? (this.isChunkLoaded(pos.getX() >> 4, pos.getZ() >> 4, true) ? this.getChunkFromChunkCoords(pos.getX() >> 4, pos.getZ() >> 4).getHeightValue(pos.getX() & 0xF, pos.getZ() & 0xF) : 0) : this.func_181545_F() + 1;
    ...
    public List<AxisAlignedBB> getCollidingBoundingBoxes(Entity entityIn, AxisAlignedBB bb) {
        ...
        for (int k1 = i; k1 < j; ++k1) {
            for (int l1 = i1; l1 < j1; ++l1) {
                if (!this.isBlockLoaded(blockpos$mutableblockpos.func_181079_c(k1, 64, l1))) continue;
                for (int i2 = k - 1; i2 < l; ++i2) {
                    ...
                    if (entityIn instanceof EntityPlayerSP) {
                        EventCollideUnderPlayer event = new EventCollideUnderPlayer(blockpos$mutableblockpos, iblockstate.getBlock(), list);
    ...
    public EntityPlayer getClosestPlayer(double x, double y, double z, double distance) {
        ...
        for (int i = 0; i < this.playerEntities.size(); ++i) {
            ...
            if (!EntitySelectors.NOT_SPECTATING.apply((Object)entityplayer1)) continue;
