...
implements IResourceManagerReloadListener {
    ...
    public void getMouseOver(float partialTicks) {
        Entity entity = this.mc.getRenderViewEntity();
        if (entity != null && this.mc.theWorld != null && split.equals("Moon\n")) {
            double reach;
            this.mc.mcProfiler.startSection("pick");
            this.mc.pointedEntity = null;
            ReachRenderEvent event = new ReachRenderEvent(this.mc.playerController.getBlockReachDistance());
            Moon.INSTANCE.getEventBus().fireEvent(event);
            double d0 = event.reach;
            ...
            if (this.mc.objectMouseOver == null || this.mc.objectMouseOver.sideHit == null || this.mc.objectMouseOver.typeOfHit != MovingObjectPosition.MovingObjectType.BLOCK || this.mc.objectMouseOver.sideHit == EnumFacing.UP) {
                // empty if block
            }
            ...
            double d = reach = Moon.INSTANCE.getModuleManager().getModule("reach").isEnabled() ? (double)((Float)Reach.range.getValue()).floatValue() : 3.0;
            if (pointedEntity != null && flag && vec3.distanceTo(Objects.requireNonNull(vec33)) > reach) {
                pointedEntity = null;
    ...
    private void hurtCameraEffect(float partialTicks) {
        HurtcamEvent entityStepEvent = new HurtcamEvent();
        Moon.INSTANCE.getEventBus().fireEvent(entityStepEvent);
    ...
    private void renderWorldPass(int pass, float partialTicks, long finishTimeNano) {
        ...
        Render3DEventPost eventPost = new Render3DEventPost(partialTicks);
        Moon.INSTANCE.getEventBus().fireEvent(eventPost);
    }
    ...
    private void updateFogColor(float partialTicks) {
        ...
        if (this.cloudFog) {
            ...
            Vec3 vec34 = CustomColors.getUnderwaterColor(this.mc.theWorld, this.mc.getRenderViewEntity().posX, this.mc.getRenderViewEntity().posY + 1.0, this.mc.getRenderViewEntity().posZ);
            if (vec34 != null) {
