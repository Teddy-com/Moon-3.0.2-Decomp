...
extends GuiScreen {
    ...
    public void initGui() {
        ...
        this.buttonList.add(new GuiButton(1337, 2, 2, 150, 20, "Disable Killaura"));
        if (this instanceof GuiChest) {
            GuiChest gui = (GuiChest)this;
            boolean bad = false;
            String name = gui.lowerChestInventory.getDisplayName().getUnformattedText().toLowerCase();
            for (String badName : ChestStealer.blacklist) {
                for (String whiteName : ChestStealer.whitelist) {
                    if (!name.contains(badName) || name.contains(whiteName)) continue;
                    bad = true;
                }
            }
            if (ChestStealer.silent.isEnabled() && (ChestStealer.customInventory.isEnabled() || !bad) && Moon.INSTANCE.getModuleManager().getModule("ChestStealer").isEnabled() && Display.isActive() && !this.mc.inGameHasFocus) {
                this.mc.inGameHasFocus = true;
                this.mc.mouseHelper.grabMouseCursor();
                Minecraft.leftClickCounter = 10000;
            }
        }
    }
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        if (this instanceof GuiChest) {
            ...
            if (ChestStealer.silent.isEnabled() && (ChestStealer.customInventory.isEnabled() || !bad) && Moon.INSTANCE.getModuleManager().getModule("ChestStealer").isEnabled() && !ChestStealer.renderGui.getValue().booleanValue()) {
                return;
            }
        }
