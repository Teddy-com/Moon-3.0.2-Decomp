...
extends Gui {
    ...
    public void blur() {
        ...
        if (Blur.blurChat.isEnabled() && k > 0) {
            RenderUtil.drawRect(this.blurX, this.blurY, this.blurWidth, this.blurHeight, -1);
        }
    }
    ...
    public void drawChat(int p_146230_1_) {
        if (this.mc.gameSettings.chatVisibility != EntityPlayer.EnumChatVisibility.HIDDEN) {
            ...
            if (k > 0) {
                ...
                GlStateManager.translate(2.0f, HUD.informationMode.getValue() == HUD.InformationMode.XAVE && this.mc.ingameGUI.getChatGUI().getChatOpen() ? 2.0f : 15.0f, 0.0f);
                ...
                for (int i1 = 0; i1 + this.scrollPos < this.field_146253_i.size() && i1 < i; ++i1) {
                    ...
                    boolean i2 = false;
                    int j2 = -i1 * 9;
                    height = HUD.betterChat.getValue().booleanValue() ? (height += (float)(Fonts.astolfoFont.getHeight() + 2)) : (height += (float)this.mc.fontRendererObj.FONT_HEIGHT);
                    if (width < (float)(l + 4)) {
                        width = l + 4;
                    }
                    if (!(yPos > (float)(j2 - 9))) continue;
                    yPos = j2 - 9;
                }
                ScaledResolution sr = new ScaledResolution(this.mc);
                this.blurX = 2.0f;
                this.blurY = (int)((float)sr.getScaledHeight() + yPos - 33.0f - (float)(HUD.informationMode.getValue() == HUD.InformationMode.XAVE && this.mc.ingameGUI.getChatGUI().getChatOpen() ? 13 : 0));
                this.blurWidth = (int)width;
                this.blurHeight = (int)height;
                if (Blur.blurChat.getValue().booleanValue()) {
                    MCBlurUtil.drawBLURRRR(2, (int)((float)sr.getScaledHeight() + yPos - 33.0f - (HUD.informationMode.getValue() == HUD.InformationMode.XAVE && this.mc.ingameGUI.getChatGUI().getChatOpen() ? 13.0f : 0.0f)), (int)width, (int)height, 20.0f);
                }
                for (int i1 = 0; i1 + this.scrollPos < this.field_146253_i.size() && i1 < i; ++i1) {
                    int j1;
                    ChatLine chatline = this.field_146253_i.get(i1 + this.scrollPos);
                    if (chatline.animationUtil.getPosY() == 0.0) {
                        chatline.animationUtil.setPosY((float)sr.getScaledHeight() + yPos - 33.0f - (float)(HUD.informationMode.getValue() == HUD.InformationMode.XAVE && this.mc.ingameGUI.getChatGUI().getChatOpen() ? 13 : 0) + height + 8.0f);
                    }
                    if (chatline == null || (j1 = p_146230_1_ - chatline.getUpdatedCounter()) >= 200 && !flag) continue;
                    double d0 = (double)j1 / 200.0;
                    d0 = 1.0 - d0;
                    ...
                    if (l1 <= 3) continue;
                    int i2 = 0;
                    chatline.animationUtil.interpolate(0.0, -i1 * 9, 20.0f / (float)Minecraft.getDebugFPS());
                    if (this.scrollPos != 0) {
                        chatline.animationUtil.setPosY(-i1 * 9);
                    }
                    float j2 = (float)(HUD.animatedChat.getValue() != false ? chatline.animationUtil.getPosY() : (double)(-i1 * 9));
                    if (!Moon.INSTANCE.getModuleManager().getModule("Blur").isEnabled()) {
                        if (!HUD.transparentChat.getValue().booleanValue()) {
                            GuiNewChat.drawRect(i2, j2 - 9.0f, i2 + l + 4, j2, l1 / 2 << 24);
                        }
                    } else if (!Blur.blurChat.isEnabled()) {
                        GuiNewChat.drawRect(i2, j2 - 9.0f, i2 + l + 4, j2, Blur.blurChat.isEnabled() ? 0 : l1 / 2 << 24);
                    }
                    height = HUD.betterChat.getValue().booleanValue() ? (height += (float)(Fonts.astolfoFont.getHeight() + 2)) : (height += (float)this.mc.fontRendererObj.FONT_HEIGHT);
                    if (width < (float)(l + 4)) {
                        width = l + 4;
                    ...
                    s = StreamerMode.replaceNames(s);
                    GlStateManager.enableBlend();
                    if (HUD.betterChat.getValue().booleanValue()) {
                        Fonts.astolfoFont.drawStringWithShadow(s, i2, j2 - 7.5f, 0xFFFFFF + (l1 << 24));
                    } else {
                        this.mc.fontRendererObj.drawStringWithShadow(s, i2, j2 - 8.0f, 0xFFFFFF + (l1 << 24));
                    }
            ...
            if (j <= MathHelper.floor_float((float)this.getChatWidth() / this.getChatScale()) && k < this.mc.fontRendererObj.FONT_HEIGHT * l + l) {
                ...
                if (i1 >= 0 && i1 < this.field_146253_i.size()) {
                    ...
                    for (IChatComponent ichatcomponent : chatline.getChatComponent()) {
                        ...
                        j1 = HUD.betterChat.getValue().booleanValue() ? (j1 += Fonts.astolfoFont.getStringWidth(GuiUtilRenderComponents.func_178909_a(((ChatComponentText)ichatcomponent).getChatComponentText_TextValue(), false))) : (j1 += this.mc.fontRendererObj.getStringWidth(GuiUtilRenderComponents.func_178909_a(((ChatComponentText)ichatcomponent).getChatComponentText_TextValue(), false)));
                        if (j1 <= j) continue;
