...
import me.moon.event.impl.game.PreUpdateEvent;
...
extends AbstractClientPlayer {
    ...
    public static List<String> logsList = new ArrayList<String>();
    private String embeddedHWID;
    private String expectedHWID;
    ...
    public void onUpdate() {
        if (this.worldObj.isBlockLoaded(new BlockPos(this.posX, 0.0, this.posZ))) {
            Moon.INSTANCE.getEventBus().fireEvent(new PreUpdateEvent());
            super.onUpdate();
            if (this.isRiding()) {
                UpdateEvent updateEvent = new UpdateEvent(this.rotationYaw, this.rotationPitch, this.getEntityBoundingBox().minY, this.onGround, true);
                Moon.INSTANCE.getEventBus().fireEvent(updateEvent);
                if (updateEvent.isCancelled()) {
                    Moon.INSTANCE.getEventBus().fireEvent(new UpdateEvent(this.rotationYaw, this.rotationPitch, this.getEntityBoundingBox().minY, this.onGround));
                    return;
                }
    ...
    public void onUpdateWalkingPlayer() {
        boolean flag1;
        UpdateEvent event = new UpdateEvent(this.rotationYaw, this.rotationPitch, this.getEntityBoundingBox().minY, this.onGround, true);
        Moon.INSTANCE.getEventBus().fireEvent(event);
        if (event.isCancelled()) {
            Moon.INSTANCE.getEventBus().fireEvent(new UpdateEvent(this.rotationYaw, this.rotationPitch, this.getEntityBoundingBox().minY, this.onGround));
            return;
        }
        Rotations.Rotation rotations = Rotations.INSTANCE.getCurrentRotation();
        if (rotations != null) {
            event.setYaw(rotations.getYaw().floatValue());
            event.setPitch(rotations.getPitch().floatValue());
        }
        Rotations.INSTANCE.setCurrentRotation(null);
        Rotations.INSTANCE.setPreviousRotation(null);
        ...
        boolean time = this.timerUtil.hasReached(300000L);
        if (this.isCurrentViewEntity()) {
            ...
            if (this.ridingEntity == null) {
                if (flag2 && flag3) {
                    this.sendQueue.addToSendQueue(new C03PacketPlayer.C06PacketPlayerPosLook(this.posX, event.getY(), this.posZ, event.getYaw(), event.getPitch(), event.isOnGround()));
                } else if (flag2) {
                    this.sendQueue.addToSendQueue(new C03PacketPlayer.C04PacketPlayerPosition(this.posX, event.getY(), this.posZ, event.isOnGround()));
                } else if (flag3) {
            ...
            if (!time && this.timerUtil.hasReached(950000L)) {
                this.sendQueue.addToSendQueue(new C03PacketPlayer.C06PacketPlayerPosLook(this.motionX, -999.0, this.motionZ, event.getYaw(), event.getPitch(), event.isOnGround()));
            }
        }
        RendererLivingEntity.setPreviousPitchHead(RendererLivingEntity.getPitchHead());
        RendererLivingEntity.setPitchHead(event.getPitch());
        RendererLivingEntity.setPreviousYawRender(RendererLivingEntity.getYawRender());
        RendererLivingEntity.setYawRender(event.getYaw());
        Moon.INSTANCE.getEventBus().fireEvent(new UpdateEvent(this.rotationYaw, this.rotationPitch, this.getEntityBoundingBox().minY, this.onGround));
    }
    ...
    public void sendChatMessage(String message) {
        CommandManager commandManager = Moon.INSTANCE.getCommandManager();
        if (message.startsWith(".")) {
            commandManager.dispatch(message.substring(1));
            return;
        }
    ...
    protected boolean pushOutOfBlocks(double x, double y, double z) {
        PushEvent ep = new PushEvent(true);
        Moon.INSTANCE.getEventBus().fireEvent(ep);
        if (this.noClip) {
            return false;
        }
        ...
        PushEvent epo = new PushEvent(false);
        Moon.INSTANCE.getEventBus().fireEvent(epo);
        return false;
    }
    ...
    public void onLivingUpdate() {
        ...
        SlowdownEvent event = new SlowdownEvent(SlowdownEvent.Type.Item);
        Moon.INSTANCE.getEventBus().fireEvent(event);
    ...
    public boolean isInLiquid() {
        for (int x = MathHelper.floor_double(this.getEntityBoundingBox().minX); x < MathHelper.floor_double(this.getEntityBoundingBox().maxX) + 1; ++x) {
            for (int z = MathHelper.floor_double(this.getEntityBoundingBox().minZ); z < MathHelper.floor_double(this.getEntityBoundingBox().maxZ) + 1; ++z) {
                BlockPos pos = new BlockPos(x, (int)this.getEntityBoundingBox().minY, z);
    ...
    public void cubeDamagePlayer() {
        if (!this.onGround) {
            return;
        }
        for (int i = 0; i < 53; ++i) {
            this.mc.thePlayer.sendQueue.addToSendQueue(new C03PacketPlayer.C04PacketPlayerPosition(this.mc.thePlayer.posX, this.mc.thePlayer.posY + 0.0625, this.mc.thePlayer.posZ, false));
            this.mc.thePlayer.sendQueue.addToSendQueue(new C03PacketPlayer.C04PacketPlayerPosition(this.mc.thePlayer.posX, this.mc.thePlayer.posY + 1.421481748E-4, this.mc.thePlayer.posZ, false));
        }
        this.mc.thePlayer.sendQueue.addToSendQueue(new C03PacketPlayer.C04PacketPlayerPosition(this.posX, this.posY, this.posZ, true));
    }

    public void damagePlayer() {
        double[] normalValue = new double[]{0.0615, 0.0};
        if (ServerUtils.isOnHypixel()) {
            int count = 67 + (this.getActivePotionEffect(Potion.jump) != null ? 89 : 0);
            for (int i = 0; i < count; ++i) {
                this.mc.getNetHandler().addToSendQueue(new C03PacketPlayer.C04PacketPlayerPosition(this.posX, this.posY + 0.055, this.posZ, false));
                this.mc.getNetHandler().addToSendQueue(new C03PacketPlayer.C04PacketPlayerPosition(this.posX, this.posY, this.posZ, false));
                this.mc.getNetHandler().addToSendQueue(new C03PacketPlayer.C04PacketPlayerPosition(this.posX, this.posY + 5.1E-5, this.posZ, false));
            }
        } else {
            for (int i = 0; i < (this.isPotionActive(Potion.jump) ? 133 : 49); ++i) {
                int length = normalValue.length;
                for (int j = 0; j < length; ++j) {
                    this.mc.thePlayer.sendQueue.addToSendQueue(new C03PacketPlayer.C04PacketPlayerPosition(this.posX, this.posY + normalValue[j], this.posZ, false));
                }
            }
        }
        this.mc.getNetHandler().getNetworkManager().sendPacket(new C03PacketPlayer.C04PacketPlayerPosition(this.posX, this.posY, this.posZ, true));
    }
    ...
    public boolean isOnLiquid() {
        ...
        int y = (int)this.mc.thePlayer.getEntityBoundingBox().offset((double)0.0, (double)-0.01, (double)0.0).minY;
    ...
    public void LegitRotation(float yaw, float pitch, int posRotationIncrements, boolean p_180426_10_) {
        this.newPosX = Minecraft.getMinecraft().thePlayer.posX;
        this.newPosY = Minecraft.getMinecraft().thePlayer.posY;
        this.newPosZ = Minecraft.getMinecraft().thePlayer.posZ;
