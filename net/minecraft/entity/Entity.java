...
implements ICommandSender {
    ...
    public void moveEntity(double x, double y, double z) {
        ...
        if (this.noClip) {
            ...
            double d2 = this.posZ;
            if (this.isInWeb) {
                SlowdownEvent event = new SlowdownEvent(SlowdownEvent.Type.Web);
                Moon.INSTANCE.getEventBus().fireEvent(event);
            ...
            if (flag || event.isCancelled() && this instanceof EntityPlayer) {
                double d6 = 0.05;
            ...
            if (this.stepHeight > 0.0f && flag1 && (d3 != x || d5 != z)) {
                StepEvent stepEvent = new StepEvent(this, true);
                Moon.INSTANCE.getEventBus().fireEvent(stepEvent);
                ...
                if (d11 * d11 + d8 * d8 >= x * x + z * z) {
                    ...
                    Moon.INSTANCE.getEventBus().fireEvent(new StepEvent(this));
                }
            }
            this.worldObj.theProfiler.endSection();
    ...
    public void moveFlying(float strafe, float forward, float friction) {
        ...
        if (f >= 1.0E-4f) {
            StrafeEvent event = new StrafeEvent(this.rotationYaw, forward, strafe);
            if (Minecraft.getMinecraft().thePlayer != null && this.getEntityId() == Minecraft.getMinecraft().thePlayer.getEntityId()) {
                Moon.INSTANCE.eventBus.fireEvent(event);
                if (event.isCancelled()) {
                    return;
                }
            }
            if (Rotations.INSTANCE.getCurrentRotation() != null) {
                event.setYaw(Rotations.INSTANCE.getCurrentRotation().getYaw().floatValue());
            }
            f = event.getStrafe() * event.getStrafe() + event.getForward() * event.getForward();
            if ((f = MathHelper.sqrt_float(f)) < 1.0f) {
                f = 1.0f;
            }
            f = friction / f;
            strafe = event.getStrafe() * f;
    ...
    public Vec3 getLookCustom(float pitch, float yaw, float partialTicks) {
    ...
    public boolean isEntityInsideOpaqueBlock() {
        ...
        Moon.INSTANCE.getEventBus().fireEvent(insideBlockRenderEvent);
