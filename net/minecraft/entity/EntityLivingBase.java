...
extends Entity {
    ...
    public static float jumpMotion = 0.3f;
    ...
    public void onEntityUpdate() {
        boolean flag1;
        if (this.lastHealth != this.getHealth() && this.lastHealth - this.getHealth() > 0.0f) {
            Moon.INSTANCE.getEventBus().fireEvent(new EntityDamageEvent(this, MathUtils.roundFloat(this.lastHealth - this.getHealth(), 1)));
        }
    ...
    private int getArmSwingAnimationEnd() {
        ...
        if (!animation.slowdown.isEnabled()) {
            return this.isPotionActive(Potion.digSpeed) ? 6 - (1 + this.getActivePotionEffect(Potion.digSpeed).getAmplifier()) * 1 : (this.isPotionActive(Potion.digSlowdown) ? 6 + (1 + this.getActivePotionEffect(Potion.digSlowdown).getAmplifier()) * 2 : 6);
        }
        if (this == Minecraft.getMinecraft().thePlayer) {
            return 10 + (Integer)animation.slowdownStrength.getValue();
        }
        return this.isPotionActive(Potion.digSpeed) ? 6 - (1 + this.getActivePotionEffect(Potion.digSpeed).getAmplifier()) * 1 : (this.isPotionActive(Potion.digSlowdown) ? 6 + (1 + this.getActivePotionEffect(Potion.digSlowdown).getAmplifier()) * 2 : 6);
    }
    ...
    public Vec3 getLook(float partialTicks) {
        ...
        AuraDev auraDev = (AuraDev)Moon.INSTANCE.getModuleManager().getModule("AuraDev");
        boolean shouldModify = killaura.isEnabled() || scaffold.isEnabled() || auraDev.isEnabled();
        float pitch = shouldModify ? RendererLivingEntity.getPitchHead() : this.rotationPitch;
        float f = prevPitch = shouldModify ? RendererLivingEntity.getPreviousPitchHead() : this.prevRotationPitch;
        if (partialTicks == 1.0f) {
            return this.getVectorForRotation(pitch, this.rotationYawHead);
        }
        float f2 = prevPitch + (pitch - prevPitch) * partialTicks;
