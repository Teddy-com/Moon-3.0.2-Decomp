...
implements Packet<INetHandlerPlayClient> {
    ...
    public S2CPacketSpawnGlobalEntity(Entity entityIn) {
        ...
        if (entityIn instanceof EntityLightningBolt) {
            if (Moon.INSTANCE.getModuleManager().getModule("LightningTracker").isEnabled()) {
                Printer.print("Lightning appeared at " + entityIn.posX + " " + entityIn.posY + " " + entityIn.posZ);
                Moon.INSTANCE.getNotificationManager().addNotification("Lightning appeared at " + entityIn.posX + " " + entityIn.posY + " " + entityIn.posZ, 3000L);
            }
            this.type = 1;
        }
    }
